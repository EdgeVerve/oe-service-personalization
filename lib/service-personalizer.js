/**
 *
 * ©2018-2018 EdgeVerve Systems Limited (a fully owned Infosys subsidiary),
...
var xapplyPersonalizationRule = function xapplyPersonalizationRuleFn(ctx, p13nRule, callback) {
  var arr = [];
  debugger;
...
function invokeHttpUrlFn(ctx, data) {
  log.debug(ctx.options, '-------- invokeHttpUrlFn. data = ', data);
  var urlStr = data.url;
...
function addPreProcessingFunction(ctx, func, instruction, fn) {
  ...
  callContext.preProcessingFns[callContext.modelName] = callContext.preProcessingFns[callContext.modelName] || [];

  if (func === 'reverseFieldReplace') {
    callContext.preProcessingFns[callContext.modelName].unshift(new ProcessingFunction(instruction, fn));
  } else {
    callContext.preProcessingFns[callContext.modelName].push(new ProcessingFunction(instruction, fn));
  }
...
function maskCharacters(ctx, instruction, cb) {
  var dsSupportHide = false;
  if (!dsSupportHide) {
    addPostProcessingFunction(ctx, 'fieldMask', instruction, maskCharactersFn);
    cb();
  } else {
    // TODO : Ds supported character mask
    cb();
  }
}

function maskCharactersFn(ctx, charMaskRules) {
  var input;
  var result = ctx.result || ctx.accdata;
