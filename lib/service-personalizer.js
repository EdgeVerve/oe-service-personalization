...
function checkPropsAndRecurse(Model, data, options, cb) {
  ...
  let isRoot = false;
  log.debug(ctx, `${isBeforeRemote ? 'beforeRemote' : 'afterRemote'}: (enter) applying personalization on properties of ${modelName}`);
  if(info && info.modelName === modelName) {
    //! this is the root model in a remote call
    isRoot = true;
  }
