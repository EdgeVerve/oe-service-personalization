...
const p13nFunctions = {
  ...
  mask(ctx, instructions) {
    return function (data, callback) { //Tests: t13
      utils.noop(data);
  ...
  addLbFilter(ctx, instruction) { //Tests: t21
    return function (data, callback) {
      utils.noop(data);
      addLbFilter(ctx, instruction, callback);
    }
  },
...
function checkRelationAndRecurse(Model, data, personalizationOptions, done) {
  ...
  // let applyOnRecord = (data, relationName, relModel, done) => {
  //   let relData = null;
  //   if(data.__data) {
  ...
  //   }
  //   let logMessage = `process relation "${relationName}" data in model "${name}"`;
      
  //   if(typeof relData !== 'undefined' && Array.isArray(relData)) {
  //     log.debug(context, `${prefix}: ${logMessage}`);
  //     async.eachSeries(relData, function relationRecordProcessor(relationRecord, cb){
  //       applyServicePersonalization(relModel, relationRecord, personalizationOptions, cb);
  //     }, function relationListItemsAsyncDoneCb(err){
  //       done(err);
  //     });
  //   }
  //   else if(typeof relData !== 'undefined' && typeof relData === 'object') {
  //     log.debug(context, `${prefix}: ${logMessage}`);
  //     applyServicePersonalization(relModel, relData, personalizationOptions, done);
  //   }
  //   else if(typeof relData === 'undefined' && isBeforeRemote) {
  //     log.debug(context, `${prefix}: ${logMessage}`);
  //     applyServicePersonalization(relModel, {}, personalizationOptions, done);
  //   }
  //   else {
  //     log.debug(context, `${prefix}: no data for relation "${relationName}" of model "${name}"`)
  //     nextTick(done);
  //   }
  // };
  ...
  if (relations) {
    ...
    let relationsIterator = function relationProcessor([ relationName, relation ], done) {
      ...
      //   async.eachSeries(data, function dataIterator(record, cb) {
      //     applyOnRecord(record, relationName, relation.model, cb);
      //   }, done);
      // }
      // else if(typeof data !== 'undefined' && typeof data === 'object') {
      //   applyOnRecord(data, relationName, relation.model, done);
      // }
      // else {
      //   nextTick(done);
      // }
      ...
      else if((relData = data[relationName])) {
        applyFlag = true;
      }
