...
var customFunction;
const { nextTick, parseMethodString, createError, formatDateTimeJoda, isDate, isString, isObject, isNumber } = require('./utils');
...
function applyServicePersonalization(modelName, data, records, options, cb) {
  ...
  doMethodGating(ctx, records, function (isAllowedMethod) {
    if (isAllowedMethod) {
      return doRoleGating(ctx, records, function (isAllowedRole) {
        if (isAllowedRole) {
          if (records.length === 0) {
...
function isValidStringMask(data, fieldMaskDefinition) {
  let lbModel = loopback.findModel(data.modelName);
  if ('stringMask' in fieldMaskDefinition) {
...
function init(app) {
  ...
  PersonalizationRule.observe('before save', function PersonalizationRuleBeforeSave(ctx, next) {
    ...
    let { personalizationRule: { postCustomFunction, fieldMask } } = data;
    ...
    if (fieldMask) {
      ...
      let fMaskDef = fMaskPropNames.reduce((carrier, fieldName) => {
        if (!carrier.hasInvalidFieldMaskDefinition) {
          ...
          let isAMask = isStringMask || isDateMask || isStringMask;
...
function doMethodGating(ctx, records, cb) {
  // let { _personalizationCache: { info, records }} = ctx;
  // Assume all allowed for now
  nextTick(() => cb(true));
}

function doRoleGating(ctx, records, cb) {
  //Assume all allowed for now

  //! we need to extract info from context cache
  nextTick(() => cb(true));
}

function getValidStaticMethod(info, isBeforeRemote) {
  let { modelName, methodName } = info;
  let Model = loopback.findModel(modelName);
  if (Model[methodName]) {
    return { isValid: true };
  }
...
function getValidInstanceMethod(info, isBeforeRemote) {
  let { modelName, methodName } = info;
  let Model = loopback.findModel(modelName);
...
function getPersonalizationMeta(ctx, info, isBeforeRemote) {
  let { methodName, isStatic, modelName } = info;
  let data = null;
  let applyPersonalizationFlag = true;
  ...
  if (isStatic) {
    switch (methodName) {
      ...
      case 'findOne':
        ...
        break;
      default:
        //!some static method we missed...
        let statInfo = getValidStaticMethod(info, isBeforeRemote);
        if (!statInfo.isValid) {
          applyPersonalizationFlag = false;
        }
        else {
          let httpMethod = ctx.req.method;
    ...
    switch (methodName) {
      case 'patchAttributes':
        ...
        break;
      default:
        let instInfo = getValidInstanceMethod(info, isBeforeRemote);
        if (!instInfo.isValid) {
          applyPersonalizationFlag = false;
        }
        else {
  ...
  return { applyPersonalizationFlag, model: theModel, data }
}
function runPersonalizationPipeline(ctx, isBeforeRemote, cb) {
  let { _personalizationCache: { info, records } } = ctx;
  // let { methodName, model, isStatic } = info;
  let pMeta = getPersonalizationMeta(ctx, info, isBeforeRemote);
  if (pMeta.applyPersonalizationFlag) {
    let { data, model } = pMeta;
    let pOptions = {
      isBeforeRemote,
      context: ctx
    }
    return applyServicePersonalization(model, data, records, pOptions, cb);
  }
  next(cb);
}
// FIXME: We don't need this method.
function performPersonalizations(ctx, isBeforeRemote, cb) {
  runPersonalizationPipeline(ctx, isBeforeRemote, cb);
}

function fetchPersonalizationRoles(ctx, cb) {
  //! doing nothing
  nextTick(cb);
}

/**
 * fetches the personalization rules
 * and checks against methodName if
 * personalizations are applicable
 * 
 * @param {string} model 
 * @param {function} cb 
 */
function runPersonalizations(ctx, isBeforeRemote, cb) {
  if (isBeforeRemote) {
    //! we have not fetched the personalization record(s)

    //fetch the personalization record and store it in ctx
    let info = parseMethodString(ctx.methodString);
    return fetchPersonalizationRecords(ctx, info.model, function (err, records) {
      if (err) {
        return cb(err);
      }
      ...
      fetchPersonalizationRoles(ctx, function (err) {
        if (err) {
          return cb(err)
        }
