...
var customFunction;
const { nextTick, parseMethodString } = require('./utils');
  ...
  if (relations) {
    ...
    let relationsIterator = function relationProcessor([relationName, relation], done) {
      ...
      if (applyFlag) {
        ...
        return fetchPersonalizationRecords(ctx, relModel, function(err, relModelP13nRecords){
          if(err) {
            return done(err);
          }
          applyServicePersonalization(relModel, relData, relModelP13nRecords, personalizationOptions, done);
        });
      }
  ...
  doMethodGating(ctx, records, function(isAllowedMethod){
    if(isAllowedMethod) {
      return doRoleGating(ctx, records, function(isAllowedRole){
        if(isAllowedRole) {
          if(records.length === 0) {
            checkRelationAndRecurse(Model, data, options, cb);
          }
          else {
            let entry = records[0];
...
function doRoleGating(ctx, records, cb) {
  //Assume all allowed for now
  nextTick(() => cb(null, true));
}

function getPersonalizationMeta(info, isBeforeRemote) {
  let { methodName, isStatic, model } = info;
  let data = null;
  ...
  if(isStatic) {
    ...
    switch(methodName) {
      case 'patchAttributes':
        ...
        break;
      default:
        let instInfo = getValidInstanceInfo(info, isBeforeRemote);
        if(!instInfo.isValid) {
          applyPersonalizationFlag = false;
        }
    }
...
function runPersonalizationPipeline(ctx, isBeforeRemote, cb) {
  ...
  let pMeta = getPersonalizationMeta(info, isBeforeRemote);
...
function performPersonalizations(ctx, isBeforeRemote, cb) {
  // doMethodGating(ctx, isBeforeRemote, function(isAllowed) {
  //   if(isAllowed) {
  //     return doRoleGating(ctx, isBeforeRemote, function(err, applyPersonalization){
  //       if(err) {
  //         return cb(err);
  //       }

  //       if(applyPersonalization) {
  //         return runPersonalizationPipeline(ctx, isBeforeRemote, cb);
  //       }
  //       cb();
  //     });
  //   }    
