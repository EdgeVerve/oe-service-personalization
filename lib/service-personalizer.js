 ...
 */
const nextTick = cb => process.nextTick(cb);
 ...
 */
function personalize(ctx, isReverse, instructions, data, done) {
  ...
  PersonalizationRule.find(findQuery, context, function(err, entries) {
    if(err) {
      ...
      if(entries.length == 0) {
        ...
        personalize(context, reverse, entries[0].personalizationRule, data, function(err) {
          if(err) {
            done(err);
          }
          else {
