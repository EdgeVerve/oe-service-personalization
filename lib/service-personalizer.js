  ...
  if (mInfo.hasModelCtorProps) {
    return async.eachSeries(mInfo.modelProps, function propPersonalizerFn([key, propDef], cb) {
      ...
      if (!isAnEmbedsOneRelationProp) {
        ...
        // begin - task02 - extract data and set
        //  applyFlag if we encounter a model
        //  constructor

        if (typeof type === 'function') {
          //! this could be a plain model constructor
          modelCtorName = type.name;
          modelCtor = type;
          unpersonalizedData = extractData(key, data);
        } else {
          //! this could be an array of model constructors
        ...
        applyFlag = typeof modelCtor.modelName === 'string';
