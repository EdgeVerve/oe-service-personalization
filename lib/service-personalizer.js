...
const utils = {
  ...
  replaceRecord(record, replacements) {
    var keys = Object.keys(JSON.parse(JSON.stringify(replacements)));
    for (var attr in keys) {
      if (keys.hasOwnProperty(attr)) {
        utils.replaceField(record, keys[attr], replacements[keys[attr]]);
      }
    }
    return record;
  }
...
const p13nFunctions = {
  fieldReplace(replacements) {
    
    return function(data, callback) {
      if(Array.isArray(data)) {
        let updatedResult = data.map(record => {
          return utils.replaceRecord(record, replacements);
        });
        data = updatedResult;
      }
      else {
        data = utils.replaceRecord(data, replacements);
      }
 ...
 */
function personalize(isReverse, instructions, data, done) {
  let tasks = Object.entries(instructions).map(([operation, instruction]) => {
    switch(operation) {
      case 'fieldReplace':
        return { type: 'fieldReplace', fn: p13nFunctions.fieldReplace(instruction) }
      default:
        return { type: operation, fn: p13nFunctions.noop }
    }
  }).sort(sortFactoryFn(isReverse));

  let asyncIterator = function( { fn }, done) {
    fn(data, function(err){
      done(err);
    });
  };

  async.each(tasks, asyncIterator, function asyncEachCb(err){
    if(err) {
      done(err)
    }
    else {
...
function checkRelationAndRecurse(Model, data, personalizationOptions, done) {
  if(Model.definition.relations) {
    relationEntries = Object.entries(Model.definitions.relations);
    async.each(relationEntries, function asyncExecutor([relationName, relationDef], asyncExecutorDoneCb){
      let relatedModelName = relationDef.model;
      //check if data has the relation
      relData = data[relationName];
      if(typeof relData !== 'undefined') {
        // apply personalization and exit function
        return applyServicePersonalization(relatedModelName, relData, personalizationOptions, function(err, persData) {
          if(err) {
            asyncExecutorDoneCb(err);
          }
          else {
            data[relationName] = persData;
            asyncExecutorDoneCb();
          }
        });
      }
      // no related data - do nothing
      process.nextTick(function() {
        asyncExecutorDoneCb();
      });
    ...
    // ! no relations defined on model - exit function
    process.nextTick(function() {
      done();
    });
  }
...
function applyServicePersonalization(modelName, data, personalizationOptions, done) {
  let { reverse, context } = personalizationOptions;
  let findQuery = { where: { modelName, enabled: true }};
  let Model = loopback.findModel(modelName);
  
  PersonalizationRule.find(findQuery, context, function(err, entries) {
    if(err) {
      done(err);
    }
    else {
      ...
      if(entries.length == 0) {
        ...
        personalize(reverse, entries[0].instructions, data, function(err) {
          if(err) {
            done(err);
          }
          else {
